...
static void distribute( double fractionalBin, double x, std::vector<double> & regions )
{
    ...
    //static int findRegionBelow( double binfreq, unsigned int howManyBins )
	int posBelow = findRegionBelow( fractionalBin, (unsigned int) regions.size() );
	int posAbove = posBelow + 1;
	
	double alpha = computeAlpha( fractionalBin, (unsigned int) regions.size() );
	
    int regsz = regions.size();
	if ( posAbove < regsz )
...
AssociateBandwidth::computeNoiseEnergy( double freq, double amp )
{
	...
	*/
    ...
    int surpsz = _surplus.size();
	if ( posAbove < surpsz && alpha != 0. && posAbove >= LowestRegion )
